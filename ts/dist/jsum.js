 ...
 */
export const multicall = async (co) => {
        ...
        if (co.target in element && typeof element[co.target] === 'function') {
            ...
            if (element.id) {
                if (element.id in hydration && hydration[element.id] === false) {
                    ...
                    if (element._start)
                        await element._start.bind(element)();
                }
            }
            try {
                ...
                return { error: error, source: element };
            }
        }
        return null;
    });
 ...
 */
export const clear_cache_for = (method_name) => {

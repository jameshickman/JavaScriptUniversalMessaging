...
export class API_REST {
    #host_url: string = '';
    #operations: Record<string, Callback[]> = {};
    #in_flight: Record<string, FetchWrapper> = {};
    ...
    constructor(host_url?: string, cb_error?: ErrorHandler) {
        ...
        this.#error_handler = cb_error || ((err) => {
            console.log(err);
        });
    }
    ...
    call(
        ...
        if (this.#operations.hasOwnProperty(signature_key)) {
            switch(http_verb) {
                case HTTP_POST_FORM:
                    return this.#call_if_not_in_flight(signature_key, url, "POST", data || null, headers, true);
                case HTTP_POST_JSON:
                    return this.#call_if_not_in_flight(signature_key, url, "POST", data || null, headers, false);
                case HTTP_DELETE:
    ...
    #apply_path_values(path: string, data: PathVars): string {
        for (const n in data) {
            const token = "{{" + n + "}}";
    ...
    #call_if_not_in_flight(
        ...
        const fetch_wrapper = (callbacks_key: string): FetchWrapper => {
            let resolved = false;
            return {
                resolved: () => {
                    return resolved;
                ...
                launch: () => {
                    fetch(this.#build_request(url, verb, data, headers, is_form))
                    .then(async (response) => {
                        resolved = true;
                        if (!response.ok) {
                            // Error!

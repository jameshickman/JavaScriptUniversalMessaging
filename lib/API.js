 ...
 * 
 * 2025-06-05:
 * Added support for Bearer Token. Typically an ancrypted JWT.
...
class API_REST {
    ...
    recall() {
        setTimeout(()=>{
            for (let i = 0; i < this.#call_cache.length; i++) {
                const call_params = this.#call_cache.pop();
                this.call(
                    call_params.signature,
                    call_params.http_verb,
                    call_params.data,
                    call_params.headers,
                    call_params.path_vars
                )
            }
        }, 500);
    }
    ...
    call(signature, http_verb, data, headers, path_vars) {
        ...
        if (this.#operations.hasOwnProperty(signature_key)) {
            switch(http_verb) {
                ...
                case HTTP_DELETE:
                    return this.#call_if_not_in_flight(signature_key, url, "DELTE", null, headers);
                case HTTP_PUT:
            ...
            return {
                ...
                launch: () => {
                    ...
                    .then(async (response) => {
                        resolved = true;
                        if (!response.ok) {
                            if (response.code === 301 && this.#cb_revalidate) {
                                this.#cb_revalidate(this, response);
                            }
                            // Error!
